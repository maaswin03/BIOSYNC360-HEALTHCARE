...
import { AlertCircle, Bell, HeartPulse, Thermometer, Droplets, Gauge, MapPin, Baby, Calendar, ClipboardList, Shield, Activity } from 'lucide-react';
...
export default function Mothercare360() {
  ...
  const fetchData = async (endpoint: string, setter: React.Dispatch<React.SetStateAction<SensorData>>) => {
    try {
  ...
  const checkAbnormal = (value: number, type: keyof typeof PREGNANCY_VITAL_RANGES) => {
    if (value === undefined || value === null) return false;
    return value < PREGNANCY_VITAL_RANGES[type].min || value > PREGNANCY_VITAL_RANGES[type].max;
  };
  ...
  const getRecommendations = () => {
    ...
    if (checkAbnormal(bloodPressureData.currSystolic, 'bloodPressureSystolic') || 
      ...
      recommendations.push({
        ...
        advice: 'Monitor for headaches or vision changes. Contact provider immediately if symptoms occur.'
      });
    }
    ...
    if (checkAbnormal(spo2Data.currSpO2, 'spo2')) {
      recommendations.push({
        ...
        advice: 'Try deep breathing exercises. Seek medical attention if below 92%.'
      });
    }
    ...
    if (checkAbnormal(temperatureData.currTemperature, 'temperature')) {
      recommendations.push({
        ...
        advice: 'Rest and hydrate. Contact provider if fever persists or exceeds 38°C.'
      });
    }
    ...
    if (checkAbnormal(rpData.currRespiratoryRate, 'respiratoryRate')) {
      recommendations.push({
        ...
        advice: 'Practice relaxation breathing. Contact provider if experiencing shortness of breath.'
      });
    }
    ...
    if (checkAbnormal(fetalHrData, 'fetalHeartRate')) {
      recommendations.push({
        ...
        advice: 'Try changing positions. If abnormal for more than 10 minutes, contact your provider.'
      });
    }
    ...
    if (fetalMovementData.currFetalMovement < 6) {
      recommendations.push({
        ...
        advice: 'Drink cold water and lie on your left side. Count kicks for 2 hours. If <6 movements, contact provider.'
      });
    }
  ...
  const vitalMetrics = [
    ...
    },
    {
      ...
      abnormal: checkAbnormal(bloodPressureData.currSystolic, 'bloodPressureSystolic') || 
               checkAbnormal(bloodPressureData.currDiastolic, 'bloodPressureDiastolic'),
  ...
  const babyDevelopmentData = [
    { week: 20, milestone: "Can hear sounds", size: "Banana", length: "25cm", weight: "300g" },
    { week: 24, milestone: "Lungs developing", size: "Grapefruit", length: "30cm", weight: "600g" },
    { week: 28, milestone: "Eyes can open", size: "Eggplant", length: "37cm", weight: "1kg" }
  ];
              ...
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm border ${
                    alertsEnabled ? 'border-red-500 text-red-500' : 'border-gray-600 text-gray-400'
                  }`}
                >
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className=" rounded-lg p-6 border ">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-xs border border-gray-700 px-2 py-1 rounded-full">Live Monitoring</span>
                </div>
                ...
                <div className=" rounded-lg p-4 border">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full border ${
                      fetalMovementData.currFetalMovement < 6 ? 'border-red-500 text-red-500' : 'border-gray-600 text-gray-400'
                    }`}>
                      {fetalMovementData.currFetalMovement || "--"} movements/hr
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-gray-400">Normal: 6-10 movements per hour</p>
                    <button 
                ...
                <div className=" rounded-lg p-6 border border-red-500">
                  ...
                  <div className="space-y-4">
                    {getRecommendations().map((rec, i) => (
                      <div key={i} className="flex gap-3 p-3 rounded-lg border border-gray-700">
                        <div className="flex-shrink-0 mt-1">
                          <AlertCircle className="h-4 w-4 text-red-500" />
                        </div>
                        <div>
                          <h4 className="font-medium">
                            {rec.parameter}: <span className="text-red-500">{rec.value}</span> 
                            <span className="text-gray-500 text-sm ml-2">(Normal: {rec.normalRange})</span>
                          </h4>
              ...
              <div className=" rounded-lg p-6 border ">
                ...
                <div className="mb-6">
                  <div className="flex justify-between mb-2">
                    <span className="text-sm text-gray-400">Week {pregnancyWeek} of 40</span>
                    <span className="text-sm text-gray-400">
                      {pregnancyWeek < 13 ? '1st Trimester' : 
                       pregnancyWeek < 27 ? '2nd Trimester' : '3rd Trimester'}
                    </span>
                  </div>
                    ...
                    {babyDevelopmentData.map((data, index) => (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <span className="text-xs bg-gray-800 px-2 py-1 rounded-full">Week {data.week}</span>
                          {data.week === pregnancyWeek && (
                            <span className="text-xs bg-white text-black px-2 py-1 rounded-full">Current</span>
                          )}
                        </div>
                  ...
                  {upcomingAppointments.map((appt, index) => (
                    <div key={index} className="p-3  rounded-lg border border-gray-700">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-xs bg-gray-800 px-2 py-1 rounded-full">{appt.date}</span>
                      </div>
                      <div className="text-sm text-gray-400">With {appt.doctor}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-black rounded-lg p-6 border border-gray-800">
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-lg border border-gray-700">
                    ...
                    <p className="text-sm text-gray-400 mb-1">City General Hospital</p>
                    <p className="text-xs text-gray-500">3.2 km away • 24/7 Maternity Ward</p>
                  </div>
